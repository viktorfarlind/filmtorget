...
import { Search, ArrowRight, Film, Disc, Ticket } from "lucide-react";
...
export default function HeroSection() {
  ...
  return (
    <div className="bg-slate-950 text-white pb-12 pt-8 px-4 shadow-2xl relative overflow-hidden border-b border-slate-800">
      ...
      <div className="max-w-2xl mx-auto text-center space-y-8 relative z-10">
        <div className="space-y-2">
          <h1 className="text-5xl sm:text-7xl font-black tracking-tighter uppercase">
            ...
            <span className="text-blue-600 not-italic">TORGET</span>
          </h1>
        ...
        >
          ...
          <div className="relative flex items-center">
            <input
              ...
              placeholder="Sök efter titel eller format..."
              autoComplete="off"
              className="w-full py-4 pl-14 pr-12 rounded-full bg-slate-800 text-white placeholder-slate-500 border border-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-slate-900 transition-all font-medium shadow-lg appearance-none"
            />
            <Search
              className="absolute left-5 h-5 w-5 text-slate-400 group-focus-within:text-blue-400 transition-colors"
              strokeWidth={2}
            />
            ...
            {searchTerm && (
              <button
                type="submit"
                className="absolute right-2 bg-blue-600 hover:bg-blue-500 p-2 rounded-full transition-all cursor-pointer active:scale-90"
              >
        ...
        <div className="flex flex-wrap justify-center gap-2 pt-2">
          {filters.map((f) => {
            const isActive =
              currentFormat === f.label ||
              (f.label === "Alla" && !currentFormat);
            const Icon = f.icon;
            ...
            return (
              <button
                ...
                className={`
                  flex items-center gap-2 px-4 py-2 rounded-full text-xs sm:text-sm font-semibold transition-all duration-300 border cursor-pointer
                  ${
                    isActive
